...
class KIT_Catalog_DbTable_Rating extends KIT_Db_Table_Abstract
{
	protected $_name = 'CATALOG_RATINGS';
	...
	public function init()
	{
		...
		$select->from(
			...
			array(
				$this->getPrimary(),
				'RATING_PRODUCTID' => "CONCAT_WS(' ', brands."
										  . $brandsTable->getFieldByAlias('title') . ', '
										  . $productsTable->getFieldByAlias('title') . ')',
				'RATING_RATING' => $this->getFieldByAlias('rating')
								   . ' / ' . $this->getFieldByAlias('ratesCnt')
			)
		);
