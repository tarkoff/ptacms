 ...
 * @version    $Id: Search.php 404 2010-05-06 21:59:07Z TPavuk $
 */
...
class Catalog_Form_Categories_Search extends KIT_Form_Abstract
{
	...
	public function __construct($select = null, $options = null)
	{
		...
		$this->setAction('/catalog/categories/search');
		...
		if (!empty($data['q'])) {
			...
			foreach ($values as $value) {
				if (!empty($value)) {
					...
					$select->where(
						'LOWER(prods.' . $prodTitleField . ') LIKE "%' . strtolower($value) . '%"'
						. ' OR '
						. 'LOWER(brands.' . $brandTitleField . ') LIKE "%' . strtolower($value) . '%"'
					);
				}
			}
		}
