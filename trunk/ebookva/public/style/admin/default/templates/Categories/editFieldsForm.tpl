<form name="{$form->name}" id="{$form->name}" action="{$form->action}" method="{$form->method}" enctype="{$form->enctype}" {if !empty($formCss)}class="{$formCss}"{/if}>
	...
	{/if}
		...
		{else}
			...
			<td align="center">
				<input type="checkbox" name="{$form->prefix}_fields[{$field->index}]" {if !empty($field->value)}checked="checked"{/if} {if !empty($field->cssClass)}class="{$field->cssClass}"{/if}{if !empty($field->disabled)} disabled="disabled"{/if}/>
			</td>
