<div id="mainMenu" class="mtb">
	<ul>
		<li class="mtbItem"><a href="{$smarty.const.ADMINURL}/Categories/">Categories</a></li>
		<li><div class="btnseparator"></div></li>
		<li class="mtbItem"><a href="{$smarty.const.ADMINURL}/Manufacturers/">Manufacturers</a></li>
		<li class="btnseparator"></li>
		<li class="mtbItem"><a href="{$smarty.const.ADMINURL}/Fields/">Fields</a></li>
		<li class="btnseparator"></li>
		<li class="mtbItem"><a href="{$smarty.const.ADMINURL}/Catalog/">Catalog</a></li>
	</ul>
</div>
<!--
<div id="mainMenu" class="mtb">
	<div class="mtbItem"><a href="{$smarty.const.ADMINURL}/Categories/">Categories</a></div>
	<div class="btnseparator"></div>
	<div class="mtbItem"><a href="{$smarty.const.ADMINURL}/Manufacturers/">Manufacturers</a></div>
	<div class="btnseparator"></div>
	<div class="mtbItem"><a href="{$smarty.const.ADMINURL}/Fields/">Fields</a></div>
	<div class="btnseparator"></div>
	<div class="mtbItem"><a href="{$smarty.const.ADMINURL}/Catalog/">Catalog</a></div>
</div>
-->