...
{if $field->type == 'Checkbox'}
	<input type="checkbox" name="{$field->name}" id="{$field->name}" {if !empty($field->value)}checked="checked"{/if} {if !empty($field->cssClass)}class="{$field->cssClass}"{/if}{if !empty($field->disabled)} disabled="disabled"{/if}/>
{/if}
