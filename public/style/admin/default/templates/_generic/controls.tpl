...
{if $field->type == $fieldType}
	<input type="password" name="{$field->name}" id="{$field->name}" value="{$field->value}" {if !empty($field->cssClass)}class="{$field->cssClass}"{/if}{if !empty($field->disabled)} disabled="disabled"{/if}/>
{/if}
